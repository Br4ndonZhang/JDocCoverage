...
class ResultWriter {
    ...
    private final String title = "JDocCoverage Report - " + DateFormat.getDateTimeInstance().format(new Date());
    private final NumberFormat numFormat = new DecimalFormat("0.0%");
    ...
    void writeResultHeader(PrintStream output) {
        output.println("<TR><TH>Name</TH><TH>method, %</TH><TH>comment, %</TH><TH>TODO</TH></TR>\n");
    }
    ...
    void writeResultLine(PrintStream output, CoverageResult result, boolean link) {
        ...
        double pc = result.commentWordsCount * 1.2 / (double) (result.commentWordsCount + result.codeWordsCount);
        int pci = (int) (pc * 100);
//        System.out.println("nf="+numFormat.format(pc) +"\tpc = "+pc+"\tpci = " +pci);
        output.print("</TD><TD>");
        output.print(result.methodCount);
        output.print(
                "</TD><TD><div class=\"progress progress-striped\"><div class=\"progress-bar progress-bar-green\" style=\"width: "
                        + pci + "%\"><span>");
        output.print(numFormat.format(pc));
        output.print(" &nbsp; (" + result.commentWordsCount + "/" + result.codeWordsCount + ")");
        output.print("</span></div></div></TD><TD>");
        output.print(result.todoCount);
//        output.print("</TD><TD>");
//        output.print(result.seeCount);
//        output.print("</TD><TD>");
//        output.print(result.linkCount);
        output.println("</TD></TR>");
    }
